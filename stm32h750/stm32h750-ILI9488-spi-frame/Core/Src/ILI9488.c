...
static inline
void color_pack(uint16_t color)
{
  buffer[0] = ((color & 0xF800) >> 8); // red
  buffer[1] = ((color & 0x07E0) >> 3); // green
  buffer[2] = ((color & 0x001F) << 3); // blue
}

/* private (static) functions */
void lcd_led(bool on)
{
  if(on) ResetPin(LCD_LED_GPIO_Port, LCD_LED_Pin);
  else SetPin(LCD_LED_GPIO_Port, LCD_LED_Pin);
}

/* private (static) functions */
void lcd_reset(void) 
{
  // ResetPin(LCD_LED_GPIO_Port, LCD_LED_Pin);
