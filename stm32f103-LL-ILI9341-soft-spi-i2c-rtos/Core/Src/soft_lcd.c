/*
 * soft_lcd, software i2c for stm32
...
void lcd_sleep(uint32_t cycle_count)
{
  uint32_t count=0;
...
lcd_t *lcd_create(GPIO_TypeDef* GPIOx, int scl, int sda, int addr, int lines) {
	...
	lcd->_addr         = addr;
	lcd->_i2c          = i2c_init(GPIOx, scl, sda);
	lcd->_lines        = lines;
...
void lcd_printf(lcd_t *lcd, const char* format, ... ) {
	int linesize = 0xA0; // max size in a 20x4 display
...
void lcd_printdigit(lcd_t *lcd, float flt)
{
  uint32_t bytes[2];

  if(flt>=0)lcd_print(lcd, "+"); // for positive

  if(flt<0)
  {
    lcd_print(lcd, "-"); // for negative
    flt = fabsf(flt); // convert to positive
  }

  bytes[1] = (uint32_t) flt;   // truncate whole numbers
  flt=(flt - bytes[1])*100;   // remove whole part of flt and shift 2 places over
  bytes[0] = (uint32_t) flt;   // truncate the fractional part from the new "whole" part

  lcd_printf(lcd, "%d", bytes[1]); // before a point
  lcd_printf(lcd, ".%d", bytes[0]); // after a point
  lcd_print(lcd, "  "); // clear space
}
