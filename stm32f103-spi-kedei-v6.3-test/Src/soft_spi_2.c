...
bitband_t mosi_2;// = BITBAND_PERIPH(&GPIOB->ODR, 6);
bitband_t miso_2;// = BITBAND_PERIPH(&GPIOB->IDR, 4);
bitband_t clock_2;// = BITBAND_PERIPH(&GPIOB->ODR, 5);
...
uint8_t soft_spi_2_transfer(uint8_t val)
{
  ...
  {
    if(clock_polar) *(clock_2) = 1; else *(clock_2) = 0;
    delay_spi_2(del);
    if(clock_phase)
    {
      bval = *(miso_2);
    ...
    {
      if(val & (1<<bit)) (*(mosi_2) = 1); else (*(mosi_2) = 0);
    }
    delay_spi_2(del);
    if(clock_polar) *(clock_2) = 0; else *(clock_2) = 1;
    delay_spi_2(del);
    if(clock_phase)
    {
      if(val & (1<<bit)) (*(mosi_2) = 1); else (*(mosi_2) = 0);
...
void soft_spi_2_wr(uint16_t val)
{
  ...
  for(i = 0; i < 8; i++)
  {
    if(val & 0x80) (*(mosi_2) = 1); else (*(mosi_2) = 0);
    val = val << 1;
    *(clock_2) = 0;
    *(clock_2) = 1;
  }
...
void soft_spi_2_wr16(uint16_t val)
{
  ...
  for(i = 0; i < 16; i++)
  {
    if(val & 0x8000) (*(mosi_2) = 1); else (*(mosi_2) = 0);
    val = val << 1;
    *(clock_2) = 0;
    *(clock_2) = 1;
  }
