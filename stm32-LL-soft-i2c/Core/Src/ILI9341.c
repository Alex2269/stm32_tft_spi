...
__STATIC_INLINE void SetOutputPin(GPIO_TypeDef *GPIOx, uint32_t PinMask)
{
  WRITE_REG(GPIOx->BSRR, (PinMask >> GPIO_PIN_MASK_POS) & 0x0000FFFFU); 
}

__STATIC_INLINE void ResetOutputPin(GPIO_TypeDef *GPIOx, uint32_t PinMask)
{
...
void lcd_reset(void) {
  ResetOutputPin(LCD_RST_GPIO_Port, LCD_RST_Pin);
  
  LL_mDelay(20);
  SetOutputPin(LCD_RST_GPIO_Port, LCD_RST_Pin);
  LL_mDelay(20);
}

void spiWrite(uint8_t data)
{
  ResetOutputPin(LCD_CS_GPIO_Port, LCD_CS_Pin);
  uint8_t i;
  for(i = 0; i < 8; i++)
  {
    if (data & 0x80)  SetOutputPin(LCD_SDA_GPIO_Port, LCD_SDA_Pin); else ResetOutputPin(LCD_SDA_GPIO_Port, LCD_SDA_Pin);
    data = data << 1;
    ResetOutputPin(LCD_SCK_GPIO_Port, LCD_SCK_Pin);  SetOutputPin(LCD_SCK_GPIO_Port, LCD_SCK_Pin);
  }
...
void spiWrite16(uint16_t data)
{
  ...
  for(i = 0; i < 16; i++)
  {
    if (data & 0x8000)  SetOutputPin(LCD_SDA_GPIO_Port, LCD_SDA_Pin); else ResetOutputPin(LCD_SDA_GPIO_Port, LCD_SDA_Pin);
    data = data << 1;
    ResetOutputPin(LCD_SCK_GPIO_Port, LCD_SCK_Pin);  SetOutputPin(LCD_SCK_GPIO_Port, LCD_SCK_Pin);
  }
  SetOutputPin(LCD_CS_GPIO_Port, LCD_CS_Pin);
...
void writeCommand(uint8_t c) {
  ResetOutputPin(LCD_DC_GPIO_Port, LCD_DC_Pin);
  spiWrite(c);
}

void writeData(uint8_t c) {
  SetOutputPin(LCD_DC_GPIO_Port, LCD_DC_Pin);
  spiWrite(c);
}
...
void ILI9341_draw_value(uint16_t value)
{
  char max_mv[16];
  utoa_local((int)value, max_mv, 10);
  if(value<1000) ILI9341_write('0');
  if(value<100)  ILI9341_write('0');
...
unsigned long utoa_local(int value, char *sp, int radix)
{
  unsigned long len;
  char tmp[16], *tp = tmp;
